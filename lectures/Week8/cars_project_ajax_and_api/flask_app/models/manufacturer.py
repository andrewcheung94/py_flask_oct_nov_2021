...
class Manufacturer:
    ...
    @classmethod
    def search_db(cls, data):
        query = "SELECT * FROM manufacturers WHERE name LIKE %(input)s;"
        data_from_db = connectToMySQL(cls.db_name).query_db(query, data)
        # No need to create instance of class since we're taking the dictionaries and returning them
        # essentially as JSON objects
        return data_from_db
    ...
    # Search the database for any manufacturers that match the text entered
    @classmethod
    def search_manufacturers(cls, data):
        query = 'SELECT * FROM manufacturers WHERE name LIKE %(input)s;'
        print(query)
        results_from_db = connectToMySQL(cls.db_name).query_db(query, data)
